...
# 타이틀 정보 읽어오기
f = codecs.open("jk_info_title.txt", "r")
title_list = []
while True:
...
# 날짜 정보 읽어오기
f = codecs.open("jk_info_date.txt", "r")
date_list = []
while True:
	line = f.readline()             # 한 줄씩 읽
	if not line: break              # break the loop when it's End Of File
...
# 조회수 정보 읽어오기
f = codecs.open("jk_info_view.txt", "r")
view_list = []
while True:
        line = f.readline()
        if not line: break
...
# href(링크) 정보 읽어오기
f = codecs.open("jk_info_href.txt", "r")
href_list = []
while True:
        line = f.readline()
        if not line: break
...
cur.execute("delete from website1_jk_info;")

idx = 0 # 리스트의 인덱스에 접근하는 변수
while idx < len(date_list):
	cur.execute("INSERT INTO website1_jk_info VALUES(?, ?, ?, ?, ?);", total_list[idx])
